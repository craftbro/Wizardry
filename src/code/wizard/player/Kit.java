...
public class Kit {
	...
	public void gearUp(){
		...
		ob = board.registerNewObjective(p.getUniqueId().toString().subSequence(0, 12)+"1", p.getUniqueId().toString().subSequence(0, 12)+"1");
		ob.setDisplayName("    "+ChatColor.YELLOW+"["+ChatColor.DARK_PURPLE+"Wizardry"+ChatColor.YELLOW+"]"+"    ");
		ob.setDisplaySlot(DisplaySlot.SIDEBAR);
		ob.getScore(Bukkit.getOfflinePlayer(ChatColor.AQUA+"    ")).setScore(60);
		ob.getScore(Bukkit.getOfflinePlayer(ChatColor.GREEN+"Your Team"+ChatColor.YELLOW+":")).setScore(59);
		...
		pl.setItem(6, new NamedStack(ChatColor.AQUA+"Melee Spells", Material.BOOK));
		pl.setItem(7, new NamedStack(ChatColor.GREEN+"Stick Spells", Material.STICK));
		pl.setItem(8, new NamedStack(ChatColor.GOLD+"Wand Spells", Material.GOLD_HOE));
	...
	if(hurt)	p.playEffect(EntityEffect.HURT);
	...
	public void tick(){
		...
		p.setLevel((int) (p.getExp()*100));
			...
			FireworkEffect effect = FireworkEffect.builder().withColor(Color.RED).withFade(Color.BLACK).with(Type.BURST).withFlicker().build();
...
switch(slot2){
	...
	public void addCondition(Condition c, int duration){
		if(conditions.containsKey(c)) return;
		conditions.put(c, duration);
	}
	
	public void giveMana(float mana){
		if(p.getExp()+(mana/100) > 1) mana = (1 - p.getExp())*100;
	...
	public void takeMana(float mana){
		if(p.getExp()-(mana/100) < 0) mana = p.getExp()*100;
	...
	public void onE(){
		...
		case 6: p.getInventory().setHeldItemSlot(7);
		break;
		case 7: p.getInventory().setHeldItemSlot(8);
		break;
		case 8: p.getInventory().setHeldItemSlot(6);
		break;
		}
	}
	...
	public void slowTick(){
		for(Condition c : conditions.keySet()){
			switch(c){
			case POISON: BasicUtil.damage(p, null, 10, DamageType.POISON);
			break;
...
//			}
	...
	private void loadBoard(){
		...
		if(health != oldHealth){ this.setHealth( p, 58);oldHealth = health;}
	
		
		if(oldLenght != conditions.size()){
			
			if(conditions.size() > 0){
			ob.getScore(Bukkit.getOfflinePlayer(ChatColor.RED+"    ")).setScore(8);
			ob.getScore(Bukkit.getOfflinePlayer(ChatColor.LIGHT_PURPLE+"Conditions"+ChatColor.YELLOW+":")).setScore(7);

			int place = 6;
			
			for(Condition c : conditions.keySet()){
				ob.getScore(Bukkit.getOfflinePlayer(c.getName())).setScore(place);
				place--;
			}
				...
				board.resetScores(ChatColor.LIGHT_PURPLE+"Conditions"+ChatColor.YELLOW+":");
	...
	private void setHealth(Player pl, int sc){
		...
		plugin.print("Setting team health of: "+pl.getName()+" in player class:" +p.getName());
		...
		if(t == null){ t = board.registerNewTeam(pl.getName());}
